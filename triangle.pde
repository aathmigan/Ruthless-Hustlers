class Triangle {
  ...
  void render(float triangleheight, boolean triggerMotion, boolean triggerGhost) {
    ...
    if (triggerMotion == true) {
      if (triggerCoinMotion == false) {
        if (triggerMouthMotionStart == true) {
          if (currentTriangleWidth + motionMouthT1 >= ((screenWidthFloat/4)/(height/triangleheight))) {
